...
> A Folk Canvas personal blog built with Next.js static export, focused on editorial layouts, owned illustration assets, GitHub Pages delivery, and an AI-assisted post publishing workflow.
...
| Content source | Current showcase routes use Folk Showcase data; new posts use Markdown + `.meta.json` publishing packages |
| Current focus | Editorial presentation, route consistency, local assets, README quality, and the AI-assisted new-post workflow |
| AI collaboration | Site implementation and workflow docs were shaped with Codex; reusable skill source lives in [`dengyie/awesome-skills`](https://github.com/dengyie/awesome-skills) |
...
- Shared content source for post cards, detail pages, RSS, and sitemap
- Publishing-package foundation with Markdown body files, companion `.meta.json`, validation, defaults, and fallback handling
- `scripts/verify-blog-package.mjs` validates that a package reaches detail, archive, category, RSS, and sitemap output
- `/submit` explains the AI publishing handoff without adding browser upload, token, or CMS behavior
...
This repository only stores the blog's site resources and publication artifacts:
...
- site design and workflow documentation

The reusable Codex skill is maintained separately in:

```text
dengyie/awesome-skills/little-lighthouse-blog-publisher
```

That skill collects article body, metadata, and optional images in stages, suggests edits and fallback choices, then writes files, builds, verifies, reviews, and commits only after final user confirmation.

GitHub Pages remains a static site. It does not accept GitHub tokens, publish directly from the browser, or store uploaded content.
...
| `/submit` | Static guide for handing content to the AI publishing workflow |
...
|-- scripts/                # Build-time helpers and publication verifiers
|-- src/
...
- `public/posts/<slug>/thumbnail.png` for an optional card image
- `public/posts/<slug>/hero.png` for an optional detail hero image
- `public/posts/<slug>/og.png` for an optional social preview image
